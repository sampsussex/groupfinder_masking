...
# Optional safety: if group catalogs could contain duplicates per id
...
# ------------------------------------------------------------
# 4) Compute deltas
#    Here delta = masked - unmasked
# ------------------------------------------------------------
merged["delta_multiplicity"] = merged[f"{mult_col}_masked"] - merged[f"{mult_col}_all"]
...
merged["delta_mass"] = merged[f"{mass_col}_masked"] - merged[f"{mass_col}_all"]
...
# Drop rows with missing values in plotted quantities
...
axes[2].set_xlabel(r"$\Delta$ multiplicity (masked - unmasked)")
axes[2].set_ylabel(r"$\Delta$ dynamical_mass")
axes[2].set_title(r"$\Delta M_{\rm dyn}$ vs $\Delta N$")
axes[2].grid(alpha=0.3)
axes[2].set_xlim(-25, -1)

# Shared colourbar
cbar = fig.colorbar(sc, ax=axes, pad=0.02, )
cbar.set_label("Unmasked group multiplicity")

plt.savefig("../plots/delta_n_delta_prop.png", dpi=300, bbox_inches="tight")
